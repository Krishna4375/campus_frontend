...
export default function CollegeManagementApp() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const prAdminResponse = await fetch('https://backend-1-8tb9.onrender.com/api/login/pr-admin', {
        method: 'POST',
    ...
    try {
      const studentResponse = await fetch('https://backend-1-8tb9.onrender.com/api/login/student', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password }),
      });
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://backend-1-8tb9.onrender.com/api/register/student', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password }),
      });
